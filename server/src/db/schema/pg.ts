...
// docs/02 A-9, Postgres dialect. This file and ./sqlite.ts differ ONLY in this header;
// schema/parity.test.ts fails the build if their shapes drift apart.
const table = pgTable;
const txt = (n: string) => text(n);
const json = <T>(n: string) => jsonb(n).$type<T>();
const ts = (n: string) => timestamp(n, { withTimezone: true, mode: 'date' });
const bool = (n: string) => boolean(n);
const int = (n: string) => integer(n);
const num = (n: string) => doublePrecision(n);
