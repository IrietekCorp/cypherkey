...
| Extension popup (eager) | **< 150 KB gzipped**, measured as the popup entry chunk plus its *static* import closure (M2-04: 94.2 KB) | This is the unlock-latency number and the one that matters. A dynamic `import()` costs nothing until its screen is reached, so it is deliberately **not** counted — including it would budget a cost the unlock path never pays and would push us to inline things that are correctly lazy |
