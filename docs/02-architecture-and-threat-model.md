...
| Extension popup (eager) | **< 230 KB gzipped**, measured as the popup entry chunk plus its *static* import closure, the stylesheets it links, and the faces those bundle (2026-09-09: 182.2 KB) | This is the unlock-latency number and the one that matters. A dynamic `import()` costs nothing until its screen is reached, so it is deliberately **not** counted — including it would budget a cost the unlock path never pays and would push us to inline things that are correctly lazy. **Raised from 150 KB on 2026-09-09**, once and deliberately, to admit Archivo and JetBrains Mono: 62 KB of the current figure is those two files. The alternative was subsetting the UI face, and the UI face is the one thing here that must never be subset — the vault renders whatever text a person saved. A bundled face is also a local file read rather than a download, and `font-display: swap` means it never blocks the first paint of an unlock |
| Extension package | **< 3 MB total** (M2-04: 767 KB) | Install size. It grows with lazily-loaded screens, which cost nothing at open but everything at download — zxcvbn's dictionaries alone are 428 KB raw |
